<?php
	...
	function sitemap_build($action, $settings, $board) {
		...
		foreach ($boards as $board) {
			$query = query(sprintf("SELECT `id`, `id` AS `thread_id`, `slug`, (SELECT `time` FROM ``posts_%s`` WHERE `thread` = `thread_id` OR `id` = `thread_id` ORDER BY `time` DESC LIMIT 1) AS `lastmod` FROM ``posts_%s`` WHERE `thread` IS NULL AND `archived` = false", $board, $board)) or error(db_error());
